...
Inner Loop (left_start): The inner loop iterates over array, but the number of iterations is not fixed like n. The inner loop depends on the size of the subarrays. The number of iterations in each iteration of the outer loop is at most $n/curr_size$. Therefore, the inner loop is $\Theta(n)$.
